...
class Ora():
    ...
    def backup(self, table, userid, remark, cond_dict):
        ...
        cond_stmt = ' and '.join(['%s=:%s' % (k, k) for k in list(cond_dict.keys())])
    ...
    def insert(self, table, column_dict):
        ...
        keys = ','.join(list(column_dict.keys()))
        values = list(column_dict.values())
        placeholder = ','.join([':%s' % (v) for v in list(column_dict.keys())])
    ...
    def update(self, table, column_dict={}, cond_dict={}):
        column_dict = self.create_params(table, column_dict)
        cond_dict = self.create_params(table, cond_dict)
        set_stmt = ','.join(['%s=:%s' % (k, k) for k in list(column_dict.keys())])
        cond_stmt = ' and '.join(['%s=:%s' % (k, k) for k in list(cond_dict.keys())])
    ...
    def get_rows(self, size=None, is_dict=True):
        ...
        if is_dict:
            ...
            for row in rows:
                dict_rows.append(dict(list(zip(dict_keys, row))))
            rows = dict_rows
        return rows
